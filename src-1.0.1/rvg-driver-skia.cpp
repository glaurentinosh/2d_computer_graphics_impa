...
uint32_t get_opt_value(const char *opt, const std::vector<std::string>& opts, uint32_t def) {
    auto len = strlen(opt);
...
void bounding_box_split(int n, int x0, int y0, int x1, int y1, int axis, std::vector<std::tuple<int, int, int, int>> &bb) {
    if (n == 0 || (x1 == x0) || (y1 == y0)) return;
    if (n == 1) {
        bb.push_back({x0, y0, x1, y1});
        return;
    }
    ...
    if (axis & 1) {
        bounding_box_split(na, x0, y0, xm, y1, axis + 1, bb);
        bounding_box_split(nb, xm, y0, x1, y1, axis + 1, bb);
    } else {
        bounding_box_split(na, x0, y0, x1, ym, axis + 1, bb);
        bounding_box_split(nb, x0, ym, x1, y1, axis + 1, bb);
    }
}
...
void render(const accelerated &a, const window &w, const viewport &v,
    ...
    auto nthreads = get_opt_value("-n:", args, omp_get_max_threads());
    nthreads = std::max<uint32_t>(nthreads, 1);
    std::vector<std::tuple<int, int, int, int>> bounding_box;
    bounding_box_split(nthreads, 0, 0, width, height, 0, bounding_box);

    // it seems somewhere under canvas->clear it is crashing on the first call to
    // sk_srgb_singleton when called inside a omp parallel region
    // thus instead of clearing the bitmap per thread we do it here
    ...
    #pragma omp parallel num_threads(nthreads)
    {
        ...
        std::tie(xmin, ymin, xmax, ymax) = bounding_box[omp_get_thread_num()];

        canvas->clipRect({float(xmin), float(ymin), float(xmax), float(ymax)});
        //canvas->clear(0);
