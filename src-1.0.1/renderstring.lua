if #arg < 5 then
    io.stderr:write("lua renderstring.lua <font file> <string> <ascender in EMs> <desdender in EMs> <height in pixels> <output PNG> [<features>]\n")
    os.exit(1)
end
...
local features
if arg[7] then
    features = harfbuzz.features(arg[7])
end
...
--local features = harfbuzz.features("tnum") -- tabular numbers
--local features = harfbuzz.features("pnum") -- proportional numbers
--local features = harfbuzz.features("onum") -- old numbers
